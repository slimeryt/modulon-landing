/**
 * PhoneLoginPage  —  /phone/login
 *
 * Same Google handoff flow as /desktop/login, but returns the session to the
 * Modulon phone app via modulon-android:// (and the shared desktop-auth claim).
 */
...
async function handoffToPhone(googleIdToken) {
  ...
  if (!ok) throw new Error('Could not hand off this sign-in to Modulon Phone.');
  window.location.href = `modulon-android://auth?code=${encodeURIComponent(nextCode)}`;
}
...
export default function PhoneLoginPage() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      await handoffToPhone(googleIdToken);
      setDone(true);
    } catch (err) {
          ...
          {done ? (
            <div className="text-center">
              ...
              <p className="text-sm text-white/40 leading-relaxed">
                You can close this tab — return to the Modulon phone app to continue.
              </p>
            </div>
          ) : (
            <>
              <h1 className="text-xl font-semibold text-white text-center mb-1">Sign in to Modulon</h1>
              <p className="text-sm text-white/40 text-center mb-8">Phone Edition</p>
              ...
              {!configured && (
                <p className="mb-5 rounded-xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-xs text-amber-200/80 leading-relaxed">
                  Firebase is not configured. Set <code className="text-white/70">VITE_FIREBASE_*</code> env vars and
                  rebuild.
                </p>
              )}
                ...
                <div className="mb-4">
                  ...
                  >
                    ...
                    <div className="flex flex-col items-start min-w-0">
                      ...
                      <span className="text-white/40 text-xs truncate w-full">{currentUser.email}</span>
                    </div>
                  </button>
              ...
              <p className="mt-6 text-center text-xs text-white/25 leading-relaxed">
                After signing in this tab will hand off your session to the Modulon phone app and can be closed.
              </p>
            </>
          )}
        </div>

        <p className="mt-6 text-center text-xs text-white/20">© {new Date().getFullYear()} Modulon</p>
      </div>
    </div>
  );
}
