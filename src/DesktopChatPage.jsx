...
const REGION_TO_CURRENCY = {
  ...
  IS:'ISK',AU:'AUD',NZ:'NZD',JP:'JPY',KR:'KRW',CN:'CNY',TW:'TWD',HK:'HKD',SG:'SGD',
  IN:'INR',RU:'RUB',TR:'TRY',ZA:'ZAR',IL:'ILS',AE:'AED',SA:'SAR',TH:'THB',MY:'MYR',
  ID:'IDR',PH:'PHP',VN:'VND',DE:'EUR',FR:'EUR',IT:'EUR',ES:'EUR',NL:'EUR',BE:'EUR',
  AT:'EUR',IE:'EUR',PT:'EUR',FI:'EUR',GR:'EUR',LU:'EUR',MT:'EUR',CY:'EUR',SI:'EUR',
  SK:'EUR',EE:'EUR',LV:'EUR',LT:'EUR',HR:'EUR',PL:'PLN',RO:'RON',CZ:'CZK',HU:'HUF',BG:'BGN',
};
  ...
  return (
    <div className={`space-y-2.5 ${className}`.trim()}>
      ...
      <div className="relative">
        <div className="h-4 w-full overflow-hidden rounded-full bg-zinc-200/90 shadow-inner dark:bg-zinc-950/80">
          <div className={`h-full rounded-full bg-gradient-to-r transition-[width] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${fillGrad}`} style={{width:`${fillWidth}%`}} />
        </div>
        <div className="pointer-events-none absolute inset-0 rounded-full ring-1 ring-inset ring-zinc-900/[0.06] dark:ring-white/[0.08]" aria-hidden />
      </div>
...
export default function DesktopChatPage() {
  ...
  const goHome = useCallback(() => {
    ...
    setAttachMenuOpen(false); setModelMenuOpen(false);
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if(!r.ok){if(!c)setApiOk(false);return;}
        const ct=(r.headers.get('content-type')||'').toLowerCase();
        if(!ct.includes('application/json')){if(!c)setApiOk(false);return;}
        const data=await r.json();
        if(!c)setApiOk(data&&data.ok===true);
      } catch{if(!c)setApiOk(false);}
    })();
    return()=>{c=true;};
  }, []);
  ...
  useEffect(()=>{
    ...
    if(settingsSection==='account'||settingsSection==='security') setSettingsSection('appearance');
  ...
  const handleSidebarChatsContextMenu = useCallback((e)=>{
    ...
    setContextMenu({x:e.clientX,y:e.clientY,conversationId:row?.getAttribute('data-conversation-id')||null});
  }, [settingsOpen, sidebarOpen]);

  useEffect(()=>{ if(settingsOpen)setContextMenu(null); }, [settingsOpen]);

  useEffect(()=>{
    if(!contextMenu)return;
    const h=(e)=>{if(contextMenuRef.current?.contains(e.target))return;setContextMenu(null);};
    document.addEventListener('pointerdown',h,true);
    const s=()=>setContextMenu(null);
    window.addEventListener('scroll',s,true);
    return()=>{document.removeEventListener('pointerdown',h,true);window.removeEventListener('scroll',s,true);};
  }, [contextMenu]);
  ...
  useLayoutEffect(()=>{
    if(!attachMenuOpen){setAttachMenuPos(null);return;}
    const gap=8,margin=8,menuWidth=208,fallbackH=112;
    const place=()=>{
      const btn=attachPlusRef.current; if(!btn)return;
      const r=btn.getBoundingClientRect();
      const menu=attachMenuRef.current;
      const h=menu?.getBoundingClientRect().height??fallbackH;
      let left=r.left;
      ...
      setAttachMenuPos({top:Math.max(margin,Math.min(top,window.innerHeight-h-margin)),left});
    };
    place();
  ...
  useEffect(()=>{
    ...
    const h=(e)=>{if(attachControlsRef.current?.contains(e.target)||attachMenuRef.current?.contains(e.target))return;setAttachMenuOpen(false);};
    document.addEventListener('pointerdown',h,true);
  ...
  useEffect(()=>{
    if(!attachMenuOpen)return;
    const h=(e)=>{if(e.key==='Escape')setAttachMenuOpen(false);};
    window.addEventListener('keydown',h);
    return()=>window.removeEventListener('keydown',h);
  }, [attachMenuOpen]);
  ...
  useEffect(()=>{
    const h=(e)=>{
      if(e.key===DAILY_USAGE_KEY||e.key===WEEKLY_USAGE_KEY||e.key===null){setDailyUsage(readDailyUsage());setWeeklyUsage(readWeeklyUsage());}
      if(e.key===EXTRA_USAGE_KEY||e.key===null)setExtraUsage(readExtraUsage());
      if(e.key===EXTRA_USAGE_CREDITS_KEY||e.key===null)setExtraUsageCredits(readExtraUsageCredits());
    };
  ...
  useEffect(()=>{
    if(settingsOpen){
      setDailyUsage(readDailyUsage());setWeeklyUsage(readWeeklyUsage());
      setExtraUsage(readExtraUsage());setExtraUsageCredits(readExtraUsageCredits());
    }
  }, [settingsOpen]);
  ...
  return (
    <div className="h-screen overflow-hidden bg-zinc-100 text-zinc-900 dark:bg-[#070708] dark:text-white font-sans selection:bg-zinc-300/40 dark:selection:bg-white/20">
      ...
      {/* ── Top-right floating pill (no Home button) ── */}
      <div
        className="fixed z-40 flex items-center gap-0.5 rounded-full border border-zinc-200/90 bg-white/90 py-1 pl-1 pr-1 shadow-sm backdrop-blur-md dark:border-white/[0.12] dark:bg-white/[0.08] dark:shadow-[0_8px_30px_-8px_rgba(0,0,0,0.65)]"
      ...
      >
        ...
        <span
          ...
          title={apiOk===true?'API connected':apiOk===false?'API offline':'Checking API…'}
          aria-label={apiOk===true?'API connected':apiOk===false?'API offline':'Checking API'}
        >
          <span className={`h-2.5 w-2.5 rounded-full ${
            apiOk===true  ? 'bg-emerald-500 shadow-[0_0_0_3px_rgba(16,185,129,0.25)] dark:shadow-[0_0_0_3px_rgba(16,185,129,0.2)]'
            :apiOk===false? 'bg-red-500 shadow-[0_0_0_3px_rgba(239,68,68,0.22)] dark:shadow-[0_0_0_3px_rgba(239,68,68,0.18)]'
      ...
      {/* ── Sidebar ── */}
      <div
        className={`fixed z-30 flex flex-col overflow-hidden transition-[width,transform] duration-200 ease-out top-[max(1rem,env(safe-area-inset-top,0px))] ${sidebarOpen?'gap-2':'gap-1.5'} ${
          sidebarOpen
            ? 'w-[min(18rem,calc(100vw-2rem))] translate-x-0'
            : 'w-14 translate-x-0'
        }`}
        style={{ left:'max(1rem,env(safe-area-inset-left,0px))', bottom:'max(1rem,env(safe-area-inset-bottom,0px))' }}
      >
        <aside
        ...
        >
          {/* Header */}
          <div className={`shrink-0 border-b border-zinc-200/80 dark:border-white/[0.06] flex items-center gap-2 ${
            sidebarOpen?'p-3 justify-between':'p-2 flex-col justify-center border-0 pb-1'
          }`}>
            <span className={`text-xs font-semibold text-zinc-700 dark:text-white/80 tracking-wide ${sidebarOpen?'':'sr-only'}`}>Chats</span>
            <button
              type="button"
              onClick={goHome}
          ...
          <div
            ...
            className={`flex-1 min-h-0 overflow-y-auto no-scrollbar p-2 space-y-1 ${sidebarOpen?'':'invisible pointer-events-none select-none'}`}
          >
            {loadingList && conversations.length===0 && <p className="text-xs text-zinc-400 dark:text-white/35 px-2 py-3">Loading…</p>}
            {!loadingList && conversations.length===0 && <p className="text-xs text-zinc-500 dark:text-white/40 px-2 py-3 leading-relaxed">No chats yet. Start a new one.</p>}
            {conversations.map(c=>(
              <div
                key={c.id}
                ...
                onKeyDown={(e)=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();selectConversation(c.id);}}}
        ...
        {/* Account / display pill */}
        {user ? (
          <div className={`shrink-0 flex items-center rounded-full border border-zinc-200/90 bg-white/90 shadow-[0_8px_30px_-8px_rgba(0,0,0,0.18)] backdrop-blur-xl dark:border-white/[0.12] dark:bg-white/[0.06] dark:shadow-[0_8px_30px_-8px_rgba(0,0,0,0.45)] transition-colors hover:bg-zinc-50/90 dark:hover:bg-white/[0.04] ${
            sidebarOpen?'gap-2 px-2.5 py-2.5':'gap-2 justify-center gap-0 px-2 py-2'
          }`}>
            {user.photoURL
              ? <img src={user.photoURL} alt="" className="h-10 w-10 shrink-0 rounded-full object-cover ring-1 ring-zinc-300/80 dark:ring-white/15 bg-zinc-200/50 dark:bg-white/10" referrerPolicy="no-referrer" />
              : <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-zinc-200 to-zinc-100 text-xs font-semibold text-zinc-700 ring-1 ring-zinc-300/80 dark:from-white/15 dark:to-white/5 dark:text-white/85 dark:ring-white/15" aria-hidden>{accountInitials(user)}</div>
            }
            <div className={`min-w-0 flex-1 ${sidebarOpen?'':'hidden'}`}>
              <p className="truncate text-sm font-medium leading-tight text-zinc-800 dark:text-white/90">{accountDisplayName(user)}</p>
          ...
          <div className={`shrink-0 border border-zinc-200/90 bg-white/90 shadow-[0_8px_30px_-8px_rgba(0,0,0,0.18)] backdrop-blur-xl dark:border-white/[0.12] dark:bg-white/[0.06] dark:shadow-[0_8px_30px_-8px_rgba(0,0,0,0.45)] ${
            sidebarOpen?'space-y-2 rounded-2xl p-3':'rounded-full p-2 flex flex-col items-center justify-center'
          }`}>
            <div className={`flex items-center justify-between gap-2 rounded-full border border-zinc-200/80 bg-zinc-50/60 dark:border-white/[0.08] dark:bg-white/[0.04] ${sidebarOpen?'px-2 py-2':'border-0 bg-transparent p-0'}`}>
            ...
            <p className={`px-1 text-[10px] leading-relaxed text-zinc-500 dark:text-white/30 ${sidebarOpen?'':'hidden'}`}>
              Firebase is off — chats stay on this device.
            </p>
          </div>
        )}
      </div>
        ...
        <div className={`mx-auto flex h-full flex-col ${isHome?'max-w-3xl':'max-w-2xl'}`}>
          ...
          {apiOk===false && (
            <div className="mb-4 rounded-xl border border-amber-500/35 bg-amber-500/[0.12] px-3 py-2.5 text-xs leading-relaxed text-amber-950 dark:border-amber-400/25 dark:bg-amber-400/10 dark:text-amber-50/95">
              <p className="font-semibold text-amber-950 dark:text-amber-100">Chat API is not available.</p>
              <p className="mt-1 text-amber-900/90 dark:text-amber-100/85">
                Run <code className="rounded bg-black/10 px-1 font-mono text-[10px] dark:bg-black/30">npm run dev:all</code> locally, or deploy{' '}
                <code className="rounded bg-black/10 px-1 font-mono text-[10px] dark:bg-black/30">server/chat-api.mjs</code>{' '}
                and set <code className="rounded bg-black/10 px-1 font-mono text-[10px] dark:bg-black/30">VITE_PUBLIC_API_ORIGIN</code>.
              </p>
            </div>
          )}
            ...
            <div className="flex min-h-0 flex-1 flex-col">
              <div className="flex flex-1 flex-col items-center justify-center gap-6 px-1 pb-4 pt-2 sm:gap-8 sm:pb-6">
                <div className="flex flex-col items-center text-center">
                  <img src={modulonIcon} alt="" decoding="async" className="mb-4 h-11 w-11 object-contain opacity-90 dark:opacity-95 sm:h-12 sm:w-12" />
          ...
          {err&&<p className="text-xs text-red-600 dark:text-red-300/90 mb-2 font-mono whitespace-pre-wrap">{err}</p>}
            ...
            <div className={`flex w-full items-center gap-0.5 border bg-white transition-shadow focus-within:ring-2 dark:bg-black/35 dark:shadow-none ${
              ...
              <div ref={attachControlsRef} className="relative shrink-0">
                <button
                  ref={attachPlusRef}
                  type="button"
                  onClick={()=>{setModelMenuOpen(false);setAttachMenuOpen(o=>!o);}}
              ...
              {attachMenuOpen&&attachMenuPos&&(
                <div ref={attachMenuRef} role="menu" aria-label="Add attachment"
                  ...
                  <button type="button" role="menuitem" className="flex w-full items-center gap-2 px-3 py-2.5 text-left text-zinc-800 hover:bg-zinc-100 dark:text-white/90 dark:hover:bg-white/[0.06]"
                    onClick={()=>{fileInputRef.current?.click();setAttachMenuOpen(false);}}>
                    <File className="h-4 w-4 shrink-0 opacity-80" aria-hidden />File
                  </button>
                  <button type="button" role="menuitem" className="flex w-full items-center gap-2 px-3 py-2.5 text-left text-zinc-800 hover:bg-zinc-100 dark:text-white/90 dark:hover:bg-white/[0.06]"
                    onClick={()=>{imageInputRef.current?.click();setAttachMenuOpen(false);}}>
                    <Image className="h-4 w-4 shrink-0 opacity-80" aria-hidden />Photo
                  </button>
                </div>
              )}
              <input
              ...
              <button
                ...
                type="button"
                onClick={()=>{setAttachMenuOpen(false);setModelMenuOpen(o=>!o);}}
                disabled={sending||apiOk===false}
                aria-expanded={modelMenuOpen}
                aria-haspopup="menu"
                aria-label={`Model: ${MODULON_CHAT_MODEL_LABEL}`}
                className="inline-flex h-11 min-w-0 max-w-[min(52vw,11rem)] shrink-0 items-center gap-0.5 rounded-full border border-transparent px-2 text-xs font-medium leading-tight text-zinc-600 transition-colors hover:border-zinc-200/80 hover:bg-zinc-100/90 focus:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400/50 disabled:pointer-events-none disabled:opacity-35 dark:text-white/65 dark:hover:border-white/15 dark:hover:bg-white/[0.06] dark:focus-visible:ring-white/25"
              >
      ...
      {/* ── Settings modal ── */}
      {settingsOpen&&(
        <div className="fixed inset-0 z-[100] flex items-stretch justify-center p-0 sm:items-center sm:p-6" role="dialog" aria-modal="true" aria-labelledby="dt-settings-title">
          <button type="button" className="absolute inset-0 bg-black/40 backdrop-blur-[2px] dark:bg-black/35 sm:bg-black/50 sm:backdrop-blur-sm dark:sm:bg-black/45" aria-label="Close settings" onClick={()=>setSettingsOpen(false)} />
          <div className="relative z-10 flex h-[100dvh] w-full max-w-none flex-col overflow-hidden border-0 bg-white/85 shadow-none backdrop-blur-2xl dark:bg-white/[0.07] dark:backdrop-blur-2xl sm:h-[min(84vh,640px)] sm:min-h-[min(380px,68vh)] sm:max-w-[min(92vw,56rem)] sm:rounded-2xl sm:border sm:border-zinc-200/60 sm:bg-white/90 sm:shadow-[0_24px_80px_rgba(0,0,0,0.12)] dark:sm:border-white/[0.1] dark:sm:bg-white/[0.08] dark:sm:shadow-[0_24px_80px_rgba(0,0,0,0.45)]">
            <div className="flex shrink-0 items-center justify-between gap-3 border-b border-zinc-200/40 bg-transparent px-4 py-3.5 dark:border-white/[0.06] sm:px-5 sm:py-4">
              <h2 id="dt-settings-title" className="text-lg font-semibold tracking-tight text-zinc-900 dark:text-white sm:text-xl">Settings</h2>
              <button type="button" onClick={()=>setSettingsOpen(false)} className="p-2 rounded-lg text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400/40 dark:text-white/45 dark:hover:bg-white/[0.08] dark:hover:text-white dark:focus-visible:ring-white/25" aria-label="Close settings">
            ...
            <div className="flex min-h-0 flex-1 flex-col bg-transparent sm:flex-row">
              <nav className="flex shrink-0 flex-row gap-1 overflow-x-auto border-b border-zinc-200/20 bg-transparent p-2 no-scrollbar dark:border-white/[0.04] sm:w-52 sm:flex-col sm:overflow-y-auto sm:border-b-0 sm:border-r sm:border-zinc-200/15 sm:bg-transparent dark:sm:border-white/[0.04]" aria-label="Settings sections">
                {[
                  ...
                  <button key={id} type="button" onClick={()=>setSettingsSection(id)} aria-current={settingsSection===id?'page':undefined}
                    ...
                    <Icon className="h-4 w-4 shrink-0 opacity-80" aria-hidden />{label}
                  </button>
                ))}
              </nav>
              <div className="min-h-0 min-w-0 flex-1 overflow-y-auto bg-transparent p-4 pb-[max(1rem,env(safe-area-inset-bottom,0px))] no-scrollbar sm:p-6 sm:pb-6">
                {settingsNotice&&(
                  <p className={`mb-5 text-xs font-mono rounded-lg px-3 py-2.5 border ${settingsNotice.startsWith('Error')?'border-red-500/30 bg-red-500/10 text-red-800 dark:border-red-500/25 dark:bg-red-500/10 dark:text-red-200/90':'border-emerald-500/30 bg-emerald-500/10 text-emerald-900 dark:border-emerald-500/25 dark:bg-emerald-500/10 dark:text-emerald-200/90'}`}>{settingsNotice}</p>
                )}
                  ...
                  <div className="w-full space-y-8">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <button type="button" role="switch" aria-checked={extraUsage} aria-label="Extra usage"
                          onClick={()=>{ const n=!extraUsage;setExtraUsage(n);writeExtraUsage(n); }}
                          className={`flex h-6 w-[3.25rem] shrink-0 items-center rounded-full p-0.5 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400/50 dark:focus-visible:ring-white/30 ${extraUsage?'justify-end bg-emerald-600 dark:bg-emerald-500':'justify-start bg-zinc-300 dark:bg-zinc-600'}`}>
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 font-mono text-xl font-semibold tabular-nums tracking-tight text-zinc-900 dark:text-white">{formatCreditsMoney(extraUsageCredits)}</p>
                        </div>
                        <button type="button" onClick={()=>setSettingsNotice('Buying extra usage is not available yet.')}
                ...
                {settingsSection==='about'&&(
                  <div className="max-w-lg space-y-3 text-sm leading-relaxed text-zinc-600 dark:text-white/45">
                    <p className="text-[10px] font-medium uppercase tracking-widest text-zinc-500 dark:text-white/35">Modulon desktop</p>
                    <p>Conversations are stored for this session and workspace. When Firebase is enabled, your account is used for sign-in only.</p>
                    <p className="font-mono text-xs text-zinc-400 dark:text-white/25">v0.1.0 — desktop build</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {/* ── Context menu ── */}
      {contextMenu&&(
        <div ref={contextMenuRef} role="menu" aria-label="Sidebar chat actions"
        ...
        >
          {(()=>{
            const cmConvo=contextMenu.conversationId?conversations.find(c=>c.id===contextMenu.conversationId):null;
            const sel=window.getSelection?.()?.toString().trim()??'';
            return(
              <>
                <button type="button" role="menuitem" disabled={!cmConvo} onClick={openConvoFromMenu} className="flex w-full items-center gap-2 px-3 py-2 text-left text-zinc-800 hover:bg-zinc-100 disabled:pointer-events-none disabled:opacity-40 dark:text-white/90 dark:hover:bg-white/[0.06]"><MessageCircle className="h-4 w-4 shrink-0 opacity-80" aria-hidden/>Open chat</button>
                <button type="button" role="menuitem" disabled={!cmConvo?.title} onClick={()=>void copyConvoTitleFromMenu()} className="flex w-full items-center gap-2 px-3 py-2 text-left text-zinc-800 hover:bg-zinc-100 disabled:pointer-events-none disabled:opacity-40 dark:text-white/90 dark:hover:bg-white/[0.06]"><ClipboardCopy className="h-4 w-4 shrink-0 opacity-80" aria-hidden/>Copy title</button>
                <button type="button" role="menuitem" disabled={!cmConvo} onClick={()=>void deleteConvoFromMenu()} className="flex w-full items-center gap-2 px-3 py-2 text-left text-red-700 hover:bg-red-500/10 disabled:pointer-events-none disabled:opacity-40 dark:text-red-300 dark:hover:bg-red-500/15"><Trash2 className="h-4 w-4 shrink-0 opacity-80" aria-hidden/>Delete chat</button>
                <div className="my-1 h-px bg-zinc-200/80 dark:bg-white/[0.08]"/>
                <button type="button" role="menuitem" disabled={!sel} onClick={()=>void copySelectionFromMenu()} className="flex w-full items-center gap-2 px-3 py-2 text-left text-zinc-800 hover:bg-zinc-100 disabled:pointer-events-none disabled:opacity-40 dark:text-white/90 dark:hover:bg-white/[0.06]"><Copy className="h-4 w-4 shrink-0 opacity-80" aria-hidden/>Copy selection</button>
                <button type="button" role="menuitem" onClick={openSettingsFromMenu} className="flex w-full items-center gap-2 px-3 py-2 text-left text-zinc-800 hover:bg-zinc-100 dark:text-white/90 dark:hover:bg-white/[0.06]"><Settings className="h-4 w-4 shrink-0 opacity-80" aria-hidden/>Open settings</button>
              </>
            );
          })()}
        </div>
